...
export const getPageLinks = async () => {
  ...
  const rows = await db
    .select({ pageLink: pageLinks, page: pages })
    .from(pageLinks)
    .leftJoin(pages, eq(pageLinks.pageId, pages.id))
    .where(eq(pageLinks.userId, session?.user.id!));
...
export const getPageLinkById = async (id: PageLinkId) => {
  ...
  const [row] = await db
    ...
    .from(pageLinks)
    .where(
      and(eq(pageLinks.id, pageLinkId), eq(pageLinks.userId, session?.user.id!))
    )
    .leftJoin(pages, eq(pageLinks.pageId, pages.id));
